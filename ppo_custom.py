...
class CustomRewardWrapper(Wrapper):
    ...
    def step(self, action):
        ...
        # Walker2d-v5의 관측값(obs) 기준
        height = obs[0]
        body_angle = obs[1]
        
        # 키를 1.4m 근처로 유지하도록 페널티 부여
        height_penalty = -0.2 * np.abs(height - 1.4)
        # 몸통이 앞으로 기울어지지 않도록 페널티 부여
        body_angle_penalty = -0.2 * np.abs(body_angle)

        new_reward = reward + height_penalty + body_angle_penalty
